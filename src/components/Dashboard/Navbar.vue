<template>
    <div v-if="user">
        <v-toolbar dark color="teal lighten-3" >
            ...
            <v-navigation-drawer dark class="imgForNavbar" absolute temporary v-model="statusNavBar">
                <v-list dark dense>
                    <v-list-tile avatar>
                        <v-list-tile-avatar>
                            <img :src="user.avatar">
                    ...
                    <v-list-tile
                            ...
                            :key="item.title"
                            v-bind:class="item.route === route ? selectedTab: ''"
                            class="pa-2"
                    >
                        <v-list-tile-action>
        ...
        data () {
        	return {
				...
				dashboardNavigation: [
					...
					{title: 'Знайти пацієнта', icon: 'insert_drive_file', method: 'logout', route: '/dashboard/findPatient'},
				],
                userNavigation: [
